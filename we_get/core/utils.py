...
def format_help(doc, errmsg):
    ...
    sys.stdout.write("Full documentation at: <http://github.com/0xl3vi/we-get>\n")
...
def msg_fetching(target):
    sys.stdout.write(
        "%s#%s Fetching data from %s'%s'%s ...\r"
        % (Fore.YELLOW, Style.RESET_ALL, Fore.BLUE, target, Style.RESET_ALL)
    )
...
def msg_item(
    item: str, items: Dict[str, str], item_color: Optional[Dict[str, str]] = None
):
    """msg_item: print item.
    @item - name.
